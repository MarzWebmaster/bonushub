...
<script>
fetch('/merchant/api/leaderboard').then(r=>r.json()).then(d=>{if(d.leaderboard){let h='';d.leaderboard.forEach((e,i)=>{let m=i==0?'🥇':i==1?'🥈':i==2?'🥉':(i+1);let name = e.customer?.name || 'Unknown';let tier = e.tier_per_merchant || 'Basic';h+='<tr><td class=\"text-center text-lg\">'+m+'</td><td class=\"font-medium\">'+name+'</td><td class=\"font-bold text-bonus-600\">'+Number(e.points).toLocaleString()+'</td><td><span class=\"badge-tier '+tier.toLowerCase()+'\">'+tier+'</span></td></tr>';});document.getElementById('lb-table').innerHTML=h;}});
</script>
@endsection
