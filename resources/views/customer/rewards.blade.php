...
<div class="page-container">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4" id="rewards-grid">Loading...</div>
</div>
<script>
    ...
    if(d.rewards){
        ...
        d.rewards.forEach(r=>{
            h+='<div class="card p-4">'
                +'<h3 class="font-bold text-surface-800">'+r.name+'</h3>'
                ...
                +'<form action="{{ route("customer.redeem") }}" method="POST" class="mt-3">'
                +'@csrf'
                +'<input type="hidden" name="reward_product_id" value="'+r.id+'">'
                +'<input type="hidden" name="merchant_id" value="'+r.merchant_id+'">'
                +'<button type="submit" class="btn-primary w-full text-sm">Redeem</button>'
                +'</form></div>';
        });
